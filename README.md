...
`tsup` compiles `src/index.tsx`, which exports all of the components in the design system, into both ES Modules and CommonJS formats as well as their TypeScript types. The `package.json` for `statium-core` then instructs the consumer to select the correct format:
...
Run `yarn build` to confirm compilation is working correctly. You should see a folder `statium-core/dist` which contains the compiled output.
...
Each file inside of `statium-core/src` is a component inside our design system. For example:
